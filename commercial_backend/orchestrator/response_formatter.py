...
def parse_analyst_response(response: str) -> dict:
    ...
    recommendation = _clean(sections.get("recommendation", ""))
...
def _clean(text: str) -> str:
    """Strip markdown bold/italic markers so text renders clean in Adaptive Cards."""
...
def _extract_bullets(text: str) -> list[str]:
    ...
    for line in text.split("\n"):
        ...
        if stripped.startswith(("- ", "* ", "• ")):
            bullets.append(_clean(stripped[2:].strip()))
