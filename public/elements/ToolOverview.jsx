    ...
    <div className="welcome-tool-overview max-w-2xl mx-auto py-8 px-4">
      <Card className="mb-6 shadow-md border-2 border-blue-100 dark:border-blue-900 bg-white dark:bg-gray-900">
        <CardHeader className="flex flex-row items-center gap-3">
          ...
          <CardTitle className="text-lg font-semibold text-gray-900 dark:text-gray-100">Welcome to the Riyadh Air Content Generator!</CardTitle>
        </CardHeader>
        <CardContent>
          {/* Video Tutorial at the top of the card */}
          <div className="flex flex-col items-center mb-6">
            <span className="inline-flex items-center gap-2 mb-3 px-6 py-2 rounded-full font-semibold text-blue-800 dark:text-blue-200 bg-blue-50 dark:bg-blue-900/40 shadow" style={{fontSize:'1rem'}}>
              <svg xmlns="http://www.w3.org/2000/svg" className="text-blue-600 dark:text-blue-300" style={{ width: '1.3em', height: '1.3em' }} fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" fill="white" className="dark:fill-gray-900"/><path stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M12 8v4m0 4h.01"/></svg>
              Watch the Tutorial: <span className="font-bold">Click the video below for a quick walkthrough</span>
            </span>
            <div className="shadow-xl border-4 border-transparent bg-gradient-to-br from-blue-50 to-white dark:from-blue-900/40 dark:to-gray-900 overflow-visible animate-glow w-full flex flex-col items-center pt-6 pb-4 rounded-xl">
              <div className="flex flex-col items-center">
                ...
                <div className="text-gray-700 dark:text-gray-300 text-sm mt-3">See how to get started and make the most of the content generator.</div>
              </div>
            </div>
          </div>
          <p className="text-gray-700 dark:text-gray-200 mb-2">
            This app provides 5 specialized tools to help you generate, refine, and translate content for Riyadh Air. Use the <span className="font-semibold text-blue-600 dark:text-blue-300">tool picker dropdown</span> at the top left of the chat to select the tool that best fits your needs.
          </p>
        ...
        {tools.map((tool, idx) => {
          ...
          return (
            <Card key={tool.name} className={`flex flex-row items-start gap-4 p-4 ${tool.color} border-0 shadow-sm`}>
              ...
              <div>
                <div className={`font-semibold text-base mb-1 text-gray-900 ${darkTitleColors[idx]}`}>{tool.name}</div>
                <div className="text-gray-700 dark:text-gray-300 text-sm">{tool.description}</div>
              </div>
            </Card>
          );
        })}
